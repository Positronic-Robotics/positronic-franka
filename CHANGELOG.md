...
- A refused stop is retried. `TrajectoryGenerator::stop_at_current()` can be rejected by the planner, and the return value was discarded — the previous plan then kept playing, so the arm went on to a target the goal had already reported as not reached. The position loop now re-asks every tick until the planner accepts.
